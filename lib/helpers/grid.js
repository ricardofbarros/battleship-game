...
grid.generate = function (dimensions) {
  ...
  for (var x = 0; x < dimensions.x; x++) {
    arr.push(generateRow(dimensions.y));
  }
...
grid.checkUnitsCollision = function (units, coords, extensiveReport) {
  var unitCoords;
...
grid.checkEveryCoordsHit = function (board, coords) {
  var coordsArr = grid.completeCoords(coords.start, coords.end);

  return coordsArr.every(function (coord) {
    if (board[coord.x][coord.y] === 'H') {
      return true;
    }
