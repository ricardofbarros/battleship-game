...
Board.prototype.export = function () {
  ...
  if (!this.phonyBoard && !this.settings.trackEnemyMisses) {
    board = board.replace(/X/gm, '0');
  }
...
Board.prototype.__getNewUnitCoords = function (unit) {
  ...
  var roulette = function () {
    ...
    // Collision detected
    if (gridHelper.checkUnitsCollision(self.units, pos)) {
      // re-run method
      return roulette();
    }
    ...
    if (gridHelper.checkEveryCoordsHit(this.grid, checkUnitCoords)) {
      ...
      for (var i = 0; i < this.units.length; i++) {
        ...
        if (checkStartCoords && checkEndCoords) {
          ...
          report.destroyed = this.units[i].id;
          break;
        }
      }
    }
