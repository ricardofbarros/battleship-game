...
Game.prototype.__generateBattleReportText = function () {
  ...
  for (var i in cpUnits) {
    textArr.push('|' + util.spacePadText(humanUnits[i].label + ' x ' + humanUnits[i].qty, lineSize) + '|');
  }

  textArr.push(util.generateBoxFooter({x: lineSize / 3}));
...
Game.prototype.__countAliveUnits = function () {
  ...
  this.cp.units.gameUnits.forEach(function (unit) {
    if (!cpUnits[unit.id]) {
      cpUnits[unit.id] = {
        label: unit.label,
        qty: 0
...
Game.prototype.drawBoards = function () {
  ...
  gameScreen
    .right(5)
    .write(util.generateBoxHeader('Enemy board', this.settings.boardDimensions));

  // Your board
  gameScreen
    .right(3)
    .write(util.generateBoxHeader('Your board', this.settings.boardDimensions));

  // Battle report board
  gameScreen
    .right(3)
    .write(util.generateBoxHeader('Battle report', {
      x: battleReport.lineSize / 3
    }) + '\n');
  ...
  gameScreen
    .move(5, 0)
    .write(util.generateBoxFooter(this.settings.boardDimensions));
...
Game.prototype.drawLastAttackReport = function () {
  ...
  // if (wasHit) {
  //   if (this.turn === 'cp') {
  //     msg = 'Your attack was a success, you hit a enemy\'s unit';
  //   } else {
  //     msg = 'The enemy attack was a success, he hit your unit';
  //   }
  // } else {
  //   if (this.turn === 'cp') {
  //     msg = 'Your attack failed';
  //   } else {
  //     msg = 'The enemy attack failed';
  //   }
  // }
  // gameScreen
  //   .move(4, 0)
...
Game.prototype.bombPosition = function (coords) {
  ...
  this.history.push({ target: target, coords: coords, hit: wasHit});
